...
LOG_MODULE_REGISTER(
    sam_log, CONFIG_SAM_LOG_LEVEL);  // Adjust CONFIG_SAM_LOG_LEVEL as needed
...
// constraints.
#define ACTION_RB_START_SIZE \
  (SAM_LOG_START_LOG_CAPACITY * sizeof(struct sam_log_rb_entry))
#define CUSTOM_RB_START_SIZE \
  (ACTION_RB_START_SIZE *    \
   2)  // Example: Assume avg custom data is similar size to entry
...
#define ACTION_RB_END_SIZE \
  (ACTION_RB_END_CAPACITY * sizeof(struct sam_log_rb_entry))
...
int sam_log_action_result(enum sam_log_status status, uint16_t custom_status,
  ...
  if (needs_space) {
    ...
    // capacity limit*, fail.
    if (sam_log_active_is_start &&
        sam_log_start_count >= SAM_LOG_START_LOG_CAPACITY) {
      LOG_WRN(
          "Start log buffer full (%u entries). Cannot add more. Discarding.",
          sam_log_start_count);
    ...
    int ret_get_old = ring_buf_get(active_action_rb, (uint8_t *)&oldest_entry,
                                   oldest_entry_size);
    ...
    if (ret_get_old != oldest_entry_size) {
      ...
      if (oldest_entry.total_custom_len > 0) {
        ...
        while (removed_total < len_to_remove) {
          size_t get_len =
              MIN(len_to_remove - removed_total, sizeof(dummy_buf));
          int ret_skip_custom =
              ring_buf_get(active_custom_rb, dummy_buf, get_len);
  ...
  // 4. Put New Data
  int ret_put_action =
      ring_buf_put(active_action_rb, (uint8_t *)&entry, required_action_space);
  if (ret_put_action != required_action_space) {
    LOG_ERR("Failed to put action entry despite space check, ret=%d",
            ret_put_action);
    return -EIO;
  }
  ...
  if (required_custom_space > 0) {
    int ret_put_custom =
        ring_buf_put(active_custom_rb, custom_data, required_custom_space);
  ...
  if (sam_log_active_is_start) {
    ...
    if (sam_log_start_count >= SAM_LOG_START_LOG_CAPACITY) {
      sam_log_active_is_start =
          false;  // Switch to end buffer for NEXT log entry
      LOG_INF(
          "Log start buffer now full (%u entries). Switching to end buffer.",
          sam_log_start_count);
    }
  }
...
int sam_log_flush_and_encode(uint8_t *output_buffer, size_t buffer_size,
  ...
  *bytes_written = 0;
  ...
  size_t start_entries =
      ring_buf_size_get(&action_rb_start) / sizeof(struct sam_log_rb_entry);
  size_t end_entries =
      ring_buf_size_get(&action_rb_end) / sizeof(struct sam_log_rb_entry);
  LOG_INF("Log Flush Placeholder: Start entries=%zu, End entries=%zu",
