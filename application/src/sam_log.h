 ...
 * @param set_default_slots If true, this action's slots_to_use becomes the new default (sets flag
 * in hdr).
 * @param custom_data       Pointer to custom data payload (can be NULL if no custom data).
 * @param custom_data_len   Length of the custom data payload in bytes (must be 0 if custom_data is
 * NULL).
 *
